...
librosa.display.specshow(
    log_spectrogram, sr=sr, hop_length=hop_length, x_axis="time", y_axis="hz", cmap="magma"
)
...
for n_mfcc in n_mfcc_list:
    for win_sec in window_lengths:
        ...
        mfcc = librosa.feature.mfcc(
            y=denoised_signal, sr=sr, n_fft=frame_length, hop_length=hop_len, n_mfcc=n_mfcc
        )
...
# ---------- SWT Feature Extraction (LIMITED TO 2 LEVELS) ----------
def extract_swt_features(signal, wavelet='db4', level=None, plot=True):
    ...
    if level is None or level > max_level:
        level = min(2, max_level)  # Limit to 2 levels maximum
    ...
    # Plot if enabled
    if plot:
        fig, axes = plt.subplots(level, 2, figsize=(14, 3 * level))
        for i, (cA, cD) in enumerate(coeffs):
            axes[i, 0].plot(cA)
            axes[i, 0].set_title(f"SWT Level {i+1} - Approximation")
            axes[i, 1].plot(cD)
            axes[i, 1].set_title(f"SWT Level {i+1} - Detail")
        plt.tight_layout()
        plt.show()

    return np.array(swt_features)

swt_features = extract_swt_features(denoised_signal, wavelet='db4', plot=True)
...
def plot_bispectrum(bispectrum, sr, title_suffix=""):
    ...
    surf = ax.plot_surface(f1, f2, bispectrum, cmap='viridis', edgecolor='none', alpha=0.8)
...
plot_bispectrum(bispectrum, sr, "Denoised Signal")
...
np.savetxt(
    bispectrum_features_file,
    bispectrum_features.reshape(1, -1),
    delimiter=",",
    header="Max,Mean,Std,Median,Energy,Entropy,MaxFreq1,MaxFreq2",
    comments=""
)
