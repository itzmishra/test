...
# ---------- SAVE ALL FEATURES INTO A SINGLE CSV (NEW ADDITION) ----------

# Create a header for the CSV
mfcc_header = [f"MFCC_{i+1}" for i in range(len(mfcc_mean))]
chroma_header = [f"Chroma_{i+1}" for i in range(len(np.mean(chroma, axis=1)))]
spectral_header = [f"SpectralCentroid_{i+1}" for i in range(len(np.mean(spectral_centroid, axis=1)))]
dwt_header = [f"DWT_{i+1}" for i in range(len(dwt_features))]
swt_header = [f"SWT_{i+1}" for i in range(len(swt_features))]
cepstrum_header = [f"Cepstrum_{i+1}" for i in range(len(cepstrum_features))]
bispec_header = [f"Bispectrum_{i+1}" for i in range(len(bispectrum_features))]
...
# Convert header to comma-separated string
header_line = ",".join(combined_header)

# Save combined features to CSV
combined_csv_file = "combined_features_test.csv"
np.savetxt(
    combined_csv_file,
...
print(f"\n=== Combined Feature CSV Created ===")
print(f"Saved combined feature vector to: {combined_csv_file}")
print(f"Total features: {combined_features.shape[0]}")

# ---------- CLEAN AND SAVE CLEANED FEATURE CSV (NEW ADDITION) ----------

import pandas as pd

# Load the raw combined features CSV
df = pd.read_csv("combined_features_test.csv")

print("\n=== Cleaning Feature CSV ===")
...
print(f"Duplicate columns removed: {before_cols - after_cols}")

# 3. Remove constant columns (columns with same value everywhere)
constant_cols = [col for col in df.columns if df[col].nunique() <= 1]
df = df.drop(columns=constant_cols)
print(f"Constant columns removed: {len(constant_cols)}")

# 4. Handle missing values (fill NaN with column mean)
missing_vals = df.isnull().sum().sum()
df = df.fillna(df.mean())
print(f"Missing values filled: {missing_vals}")

# 5. Save cleaned CSV
cleaned_csv_file = "combined_features_test_cleaned.csv"
df.to_csv(cleaned_csv_file, index=False)

print(f"\n=== CLEANING COMPLETE ===")
print(f"Saved cleaned feature CSV to: {cleaned_csv_file}")
